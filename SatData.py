...
class SatData:
    ...
    def save_as_csv(self, dbns):
        ...
        for school in self.data:
            if school["dbn"] in dbns:  # Access the correct field name for DBN
                row = [
                    school["dbn"],  # Access the correct field name for DBN
                    f'"{school["school_name"]}"',  # Enclose in double quotes to handle commas in school names
                    school["num_of_sat_test_takers"],
                    school["sat_critical_reading_avg_score"],
                    school["sat_math_avg_score"],
                    school["sat_writing_avg_score"]
                ]
